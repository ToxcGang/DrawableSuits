...
- The editor cursor is dynamic and rendered on a dedicated topmost cursor canvas: Paint and Erase show a hollow brush ring sized to the current editable target, while UI hover, invalid targets, Decal, Text, Eyedropper, and normal navigation show a small white dot. Cursor diagnostics should report `sortingOrder=32767`; the editor canvas uses a lower order so the cursor cannot render behind it.
...
- If the cursor is missing, confirm the installed package is 0.5.18 or newer and check `CursorCanvasState` diagnostics. The cursor canvas should be active while the editor is open, use `ScreenSpaceOverlay`, have `overrideSorting=True`, and report `requestedSortingOrder=32767` plus `sortingOrder=32767`. If it ever reports `sortingOrder=-32768`, the package is older than the sorting-order fix.
- If the cursor is still a filled square or colored world blob, confirm the installed package is 0.5.18 or newer and check `DynamicCursorUpdated` diagnostics. Paint and Erase should use `mode=BrushRing`; Decal, Text, Eyedropper, UI hover, and invalid targets should use `mode=Dot`.
