...
- The temporary edit preview preserves the generated stamp aspect after crop and width/height stretch, so the panel preview should match the placement preview and final stamp shape. Decal previews use cached CPU source pixels for real-time preview-quality rendering while placement previews and final stamps remain full quality.
...
- If Decal or Sticker temporary edits do not appear, final Decal stamps use the original image, the edit-panel preview looks stretched, or temporary edit preview updates feel too slow, confirm the installed package is 0.5.56 or newer and check `PlacementEditPanelOpened`, `PlacementEditChanged`, `PlacementSourcePixelsCached`, `PlacementSourcePixelsCacheHit`, `DecalEditPreviewJobQueued`, `DecalEditPreviewJobCompleted`, `DecalEditPreviewJobDiscarded`, `DecalEditPreviewJobApplied`, `PlacementEditPreviewUpdated`, `PlacementEditPreviewCacheHit`, `PlacementEditPreviewSkipped`, `PlacementEditedStampGenerated`, `PlacementEditReset`, and `PlacementEditPanelClosed`. Decal edit-panel previews render as full-quality async CPU jobs and then upload on the main thread; Sticker previews remain immediate. Placement previews/final stamps remain full quality. These edits are session-only and never modify decal files or built-in sticker sources.
