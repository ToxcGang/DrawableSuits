...
- Controller support: use the pause-menu button or shortcut to open the editor, move the virtual cursor with the left stick, paint with right trigger, rotate with bumpers, cycle tools with `Y`, undo with `X`, save with Start, and submit the selected UI control with `A`.
...
- 3D suit preview rendered inside the editor with a dedicated preview camera and RenderTexture.
...
Lethal Company v81 runs with Unity's Input System path. DrawableSuits 0.3.2 reads `F8`, `F9`, `F10`, Escape, mouse movement, mouse buttons, mouse wheel, and controller controls through that path so `UnityEngine.Input` errors should not repeat in `LogOutput.log`.

## Known Limits

- The editor uses the local player model as the baked preview mesh, then renders it through a dedicated preview camera inside the editor panel. If no player model is available yet, the diagnostics overlay still opens and reports that missing dependency.
...
- The `0.3.2` editor uses deterministic anchored UI controls and fixed slider child rects. If sliders appear as large orange blocks, make sure the installed package is `0.3.2` or newer and check `diagnostics.log` for `SliderBuilt` lines.
- If the player preview is missing, check `diagnostics.log` for `RebuildPreview complete`, `previewCamera`, and `renderTexture` entries.
...
- While the editor is open, DrawableSuits disables the local player's movement action map and locks movement/look/interact flags so controller input drives the editor instead of jumping or walking the player.
- Controller `A` no longer applies the suit globally; it only submits the currently selected UI control.
...
- If `LogOutput.log` contains repeated `Legacy key polling failed` or `InvalidOperationException` entries from DrawableSuits, make sure the installed package is `0.3.2` or newer.
