...
- If Decal or Sticker temporary edits do not appear, final Decal stamps use the original image, the edit-panel preview looks stretched, or temporary edit preview updates feel too slow, confirm the installed package is 0.5.57 or newer and check `PlacementEditPanelOpened`, `PlacementEditChanged`, `PlacementSourcePixelsCached`, `PlacementSourcePixelsCacheHit`, `DecalEditPreviewFastUpdated`, `DecalEditPreviewFastCacheHit`, `DecalEditPreviewFastSkipped`, `PlacementEditPreviewUpdated`, `PlacementEditPreviewCacheHit`, `PlacementEditPreviewSkipped`, `EditedDecalFullQualityGenerated`, `PlacementEditedStampGenerated`, `PlacementEditReset`, and `PlacementEditPanelClosed`. Decal edit-panel previews render at preview resolution for stable FPS; placement previews/final stamps remain full quality. These edits are session-only and never modify decal files or built-in sticker sources.
