...
- The temporary edit preview preserves the generated stamp aspect after crop and width/height stretch, so the panel preview should match the placement preview and final stamp shape. Decal edit-panel previews render at preview resolution with cached CPU source pixels for stable FPS, while placement previews and final stamps remain full quality. Full-quality Decal/Sticker placement stamps are not generated while edit modals are open and are deferred until a valid target preview or final stamp needs them.
...
- Active editor diagnostics report full proxy mesh/collider state through `WorldAvatarProxyRebaked` and `WorldAvatarProxyPoseFrozen`; `PartClassifierBuilt` should not appear during normal editor use.
...
- If third person shows first-person arms, a giant helmet, held items, or another partial rig, check `World renderer candidate`, `Hidden nearby first-person overlay renderer`, `World editor visible renderer candidate`, and `WorldAvatarProxyRebaked` lines. The selected renderer should be a body/suit renderer and the proxy should use only the player-specific DrawableSuits material for suit-compatible submeshes.
- If the third-person editor avatar moves or breathes while open, confirm the installed package is 0.5.59 or newer and check `WorldAvatarProxyPoseFrozen`. `WorldAvatarProxyRebaked` should appear only for explicit preview setup/rebuilds, not every frame while idle.
...
- If you cannot see the local suit in third person, check `diagnostics.log` for `WorldThirdPerson setup`, `WorldAvatarProxyRebaked`, `WorldAvatarProxyPoseFrozen`, and `WorldEditorCamera updated`.
...
- If Decal or Sticker temporary edits do not appear, final Decal stamps use the original image, the edit-panel preview looks stretched, or temporary edit preview updates feel too slow, confirm the installed package is 0.5.58 or newer and check `PlacementEditPanelOpened`, `PlacementEditChanged`, `PlacementSourcePixelsCached`, `PlacementSourcePixelsCacheHit`, `DecalEditPreviewFastUpdated`, `DecalEditPreviewFastCacheHit`, `DecalEditPreviewFastSkipped`, `PlacementPreviewSkippedForModal`, `PlacementPreviewStampDeferred`, `PlacementEditPreviewUpdated`, `PlacementEditPreviewCacheHit`, `PlacementEditPreviewSkipped`, `EditedDecalFullQualityGenerated`, `PlacementEditedStampGenerated`, `PlacementEditReset`, and `PlacementEditPanelClosed`. Decal edit-panel previews render at preview resolution for stable FPS; placement previews/final stamps remain full quality, but full-quality placement stamps should not be generated while the edit modal is open. These edits are session-only and never modify decal files or built-in sticker sources.
