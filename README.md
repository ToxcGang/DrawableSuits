...
- If Decal or Sticker temporary edits do not appear, final Decal stamps use the original image, the edit-panel preview looks stretched, or temporary edit preview updates feel too slow, confirm the installed package is 0.5.53 or newer and check `PlacementEditPanelOpened`, `PlacementEditChanged`, `PlacementEditPreviewDeferred`, `PlacementEditPreviewRebuilt`, `PlacementEditPreviewSkipped`, `PlacementEditedStampGenerated`, `PlacementEditReset`, and `PlacementEditPanelClosed`. These edits are session-only and never modify decal files or built-in sticker sources.
