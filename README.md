...
- If Decal or Sticker temporary edits do not appear, final Decal stamps use the original image, the edit-panel preview looks stretched, or temporary edit preview updates feel too slow, confirm the installed package is 0.5.55 or newer and check `PlacementEditPanelOpened`, `PlacementEditChanged`, `PlacementSourcePixelsCached`, `PlacementSourcePixelsCacheHit`, `PlacementEditPreviewUpdated`, `PlacementEditPreviewCacheHit`, `PlacementEditPreviewSkipped`, `PlacementEditedStampGenerated`, `PlacementEditReset`, and `PlacementEditPanelClosed`. Edit-panel previews are real-time preview-quality renders; Decal previews avoid per-pixel Unity texture API sampling by reusing cached CPU pixels, and placement previews/final stamps remain full quality. These edits are session-only and never modify decal files or built-in sticker sources.
